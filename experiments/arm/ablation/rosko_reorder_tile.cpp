...
int main( int argc, char** argv ) {
	...
	double ret = 0;
	float density = 1.0 - (((float) sp) / 100.0);

	clock_gettime(CLOCK_REALTIME, &start);

	for(int i = 0; i < ntrials; i++) {
		ret += cake_sp_sgemm(A, B, C, M, N, K, p, cake_cntx, density);
	}
	...
	printf("rosko+reorder+sp_tiling sp_sgemm time: %f \n", ret/ntrials); 
	...
	if(write_result) {
	    ...
	    FILE *fp;
	    fp = fopen(fname, "a");
	    fprintf(fp, "rosko+reorder+sp_tiling,%d,%d,%d,%d,%f\n",M,K,N,sp,ret/ntrials);
	    fclose(fp);
	}
