...
int run_sparse_matrix_dense_matrix_multiply_example(const cl::sycl::device &dev,intType nrows, 
    intType ncols, std::int64_t columns, double density_val, int ntrials, char* argv[])
{
    ...
    try {
        ...
        FILE *fp1;
        fp1 = fopen(fname, "a");
        fprintf(fp1, "mkl_csr,%d,%d,%d,%d,%f,%f\n",nrows,ncols,columns,10,atof(argv[4]),diff_t / ntrials);
        fclose(fp1);
...
int main(int argc, char* argv[]) {
    ...
    double density = 1.0 - (((double) sp) / 100.0);
    // double density = atof(argv[4]);
    int ntrials = atof(argv[5]);
