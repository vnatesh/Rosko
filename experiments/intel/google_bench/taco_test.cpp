...
//   g++ -std=c++11 -fopenmp -pthread -O3 -DNDEBUG -DTACO -I include -Lbuild/lib taco_test.cpp -o spmm -ltaco
...
int main(int argc, char* argv[]) {
  ...
  for (int i = 0; i < M; ++i) {
    for (int j = 0; j < K; ++j) {
      int x = rand();
      if((x > ((float) RAND_MAX)*sp) && (cnt != nnz)) {
        fprintf(fp1, "%d %d %f\n",i,j,unif(gen));
        cnt++;
      }
    }
