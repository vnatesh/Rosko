...
def plot_rosko_vs_intel_test(fname = 'rosko_vs_intel_test', ntrials = 10):
	...
	layers = df1[(df1['algo'] == 'rosko') & (df1['N'] == feat_lens[j]s[0])]['layer']._values
	cores = range(2,11)
	error = 0
	#
	rosko_times = []
	...
	for sp in sps:
		...
		for p in cores:
			N = df1[(df1['algo'] == 'rosko') & (df1['file'] == file)]['N']._values[0]
			M = df1[(df1['algo'] == 'rosko') & (df1['file'] == file)]['M']._values[0]
			K = df1[(df1['algo'] == 'rosko') & (df1['file'] == file)]['K']._values[0]
			nz = (1.0 - (float(sp) / 100.0))*M*K
			flops.append(nz) 
			df2 = pandas.read_csv('reports_intel_test/report_rosko_%s-%d.csv' % (file,feat_lens[j]) ,skiprows=17,skipfooter=16)
			cpu_time = df1[(df1['algo'] == 'rosko') & (df1['file'] == file) & (df1['N'] == feat_lens[j])]['time']._values[0]
			dram_bw = df2['Average']._values[0]
			gflops_rosko.append((nz*N) / cpu_time / 1e9)
			df2 = pandas.read_csv('reports_intel_test/report_rosko_%s-%d.csv' % (file,feat_lens[j]),skipfooter=20)
			elapsed = df2[df2['Metric Name'] == 'Elapsed Time']['Metric Value']._values[0]
			#
			df3 = pandas.read_csv('reports_intel_test/report_setup_%s-%d.csv' % (file,feat_lens[j]) ,skiprows=17,skipfooter=16)
			dram_bw1 = df3['Average']._values[0]
			df3 = pandas.read_csv('reports_intel_test/report_setup_%s-%d.csv' % (file,feat_lens[j]),skipfooter=20)
			elapsed1 = df3[df3['Metric Name'] == 'Elapsed Time']['Metric Value']._values[0]
			dram_bw = ((dram_bw*elapsed) - (dram_bw1*elapsed1)) / (elapsed-elapsed1)
			dram_bw_rosko.append(dram_bw)
			time_rosko.append((elapsed-elapsed1) / ntrials)
		dram_io_rosko_pred = df1[(df1['algo'] == 'rosko_dram') & (df1['N'] == feat_lens[j]) & (df1['p'].isin(cores))]['time']._values
...
def plot_rosko_vs_arm_test(fname = 'rosko_vs_arm_test', ntrials = 1):
	...
	for sp in sps:
		...
		for p in cores:
			...
			# elapsed -= df1[(df1['algo'] == 'setup') & (df1['layer'] == layer) & (df1['N'] == feat_lens[j])]['time']._values[0]
			...
			cpu_time = df1[(df1['algo'] == 'armpl') & (df1['file'] == file) & (df1['N'] == feat_lens[j])]['time']._values[0]
			tmp = (((int(re.search(r'\d+', a[5]).group())*64.0))) / 1e9
			dram_io_armpl.append(tmp / ntrials)
			dram_bw_armpl.append(tmp / ntrials / cpu_time)
			gflops_armpl.append((M*K*N) / cpu_time / 1e9)
		dram_io_rosko_pred = df1[(df1['algo'] == 'rosko_dram') & (df1['N'] == feat_lens[j])]['time']._values
		dram_bw_rosko_pred = list(dram_io_rosko_pred / np.array(time_rosko))
		error += np.mean(dram_bw_rosko_pred / np.array(dram_bw_rosko))
		rosko_pred_dram.append(dram_bw_rosko_pred)
		rosko_pred_times.append(time_rosko)
...
def plot_rosko_vs_arm_dnn(fname = 'rosko_vs_arm_dlmc', ntrials = 1):
	...
	for sp in sps:
		...
		for layer in layers:
			...
			cpu_time = df1[(df1['algo'] == 'rosko') & (df1['layer'] == layer) & (df1['N'] == feat_lens[j])]['time']._values[0]
			tmp = (((int(re.search(r'\d+', a[5]).group())*64.0))) / 1e9
			tmp -= (((int(re.search(r'\d+', b[5]).group())*64.0))) / 1e9
			#
			# tmp = (((int(re.search(r'\d+', a[9]).group())*4.0))) / 1e9
...
def plot_rosko_vs_intel_dnn(fname = 'rosko_vs_arm_dlmc', ntrials = 10):
	...
	df1 = pandas.read_csv('results_intel_test')
	sps = [80, 85, 90, 95, 98]
	layers = df1[(df1['algo'] == 'cake') & (df1['N'] == feat_lens[j]s[0])]['layer']._values
	cores = range(2,11)
	error = 0
	#
	for sp in sps:
