...
int run_sparse_matrix_dense_matrix_multiply_example(const cl::sycl::device &dev,intType nrows, 
    intType ncols, std::int64_t columns, double density_val, int ntrials)
{
...
int main(int argc, char* argv[]) {
    ...
    for (auto it = list_of_devices.begin(); it != list_of_devices.end(); ++it) {
        ...
        if (my_dev_is_found) {
            ...
            status = run_sparse_matrix_dense_matrix_multiply_example<float, std::int32_t>(my_dev, M, K, N, density, ntrials);
            if (status != 0)
                return status;
